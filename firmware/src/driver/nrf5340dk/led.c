...
/* nRF5340-DK has 4 discrete LEDs on led0..led3 aliases (active-low) */
...
static const struct gpio_dt_spec led_orange = GPIO_DT_SPEC_GET(DT_ALIAS(led3), gpios);
...
static void thread_entry(void *p1, void *p2, void *p3)
{
	...
	while (true) {
		...
		if (atomic_test_bit(led_bits, LED_ORANGE)) {
			(void)gpio_pin_toggle_dt(&led_orange);
		} else {
...
int led_initialize(void)
{
	...
	if (ret < 0) {
		LOG_ERR("Failed to init orange LED (err %d)", ret);
		return ret;
	}
